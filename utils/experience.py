...
SEED_LESSONS: list[dict] = [
    ...
    },
    {
        ...
        "lesson": "涉及'是否遮蔽/覆盖/相交'的判定函数，必须先用已知案例验证："
                  "① 明显遮蔽（云团中心在导弹-目标连线上）→ 应返回 True；"
                  "② 明显不遮蔽（云团在 100km 外）→ 应返回 False。"
                  "全零结果 90% 是判定条件写反（如距离<threshold 写成 >threshold）"
                  "或坐标系不一致（ENU vs 地心坐标混用）。",
        "source": "2025A Q1-Q5 实测",
    },
    {
        "problem_type": "优化",
        "keywords": ["读取失败", "文件不存在", "静默降级", "默认值", "FileNotFoundError"],
        "lesson": "数据读取失败会静默使用全 0 默认值，导致结果无意义且极难定位。"
                  "代码必须：① 读取失败时抛异常而非 except 后静默降级；"
                  "② 打印读取的行数和前 3 行数据验证；"
                  "③ 数据为空时立即 raise 终止——宁可报错也不用假数据。",
        "source": "2025A Q3 实测",
    },
    {
        "problem_type": "优化",
        "keywords": ["空跑", "metrics.json", "无产出", "fake_success"],
        "lesson": "metrics.json 为空 + stdout 无数值输出 = 空跑（只 print 不 compute）。"
                  "常见原因：① 所有计算都在 if __name__ 块内但入口没调用；"
                  "② 条件分支跳过了核心计算；③ 写入了错误的文件名。"
                  "代码必须保证：即使无优化结果，也要输出当前参数下的评估值。",
        "source": "通用",
    },
]
